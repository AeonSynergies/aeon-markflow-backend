...
⚠️ **A known follow-up, deliberately not done in this same change**: `GET /me` (`src/routes/me.routes.ts`) was still its own open, unmerged PR when `BD_MARKETING` was introduced here, and its `MeResponse.org_access.roles` OpenAPI schema hardcodes its own separate 6-role enum (`src/config/openapi.ts`, independent of the `ROLES` constant — same pattern as the `UserAccessGrant` enums this change *did* update). Editing that other PR's branch felt like the wrong call — it's mid-review with its own auto-merge armed, and this task never asked for it. Whichever of the two PRs merges second needs a small follow-up adding `BD_MARKETING` to that one remaining enum, or `GET /me` will under-report a BD-Marketing user's own role back to the frontend that needs it for nav-gating.
