...
- Hosting (when ready to deploy, not needed for local dev): new AWS account (shared with Onboard) — Amazon ECS Express Mode (backend — see the superseded note below), S3 + CloudFront (frontend), MongoDB Atlas, ElastiCache/Upstash (Redis), Secrets Manager, Amazon ECR (container images)
- ⚠️ **`apprunner.yaml`/`apprunner-worker.yaml` are superseded, not deleted.** AWS closed App Runner to new customers as of 2026-04-30, so neither file is this backend's deployment path anymore — deploying now means Amazon ECS Express Mode instead (see the Dockerfile/`ecr-publish.yml` bullets below). Both files are left in the repo purely for reference (the run commands and "worker needs its own health check" reasoning they document are still accurate, just for a different target platform) — not attempted to delete or rewrite them for ECS, since an existing App Runner deployment (if one was ever actually stood up from them) shouldn't be assumed dead until someone confirms that migration happened. Their own bullets below are left as originally written, for the same reason.
...
- **`Dockerfile`** (repo root) — the real current deployment path's build artifact: a three-stage build (`deps`: production-only `node_modules`; `build`: full deps + `tsc`; `runtime`: `node:20-slim` with just `dist/` + production `node_modules`, non-root user) matching `apprunner.yaml`'s Node 20 version. One image serves both processes, same split as the two App Runner configs it supersedes: `CMD ["node", "dist/server.js"]` is the default (the API server), and an ECS Express Mode worker service overrides the container command to `node dist/worker.js` instead — no separate image/build. Includes a `HEALTHCHECK` hitting `GET /health`, which both `dist/server.js` (`src/app.ts`) and `dist/worker.js` (via `src/healthCheckServer.ts`) serve, so it works unmodified regardless of which command a given deployment runs. Build/push verified locally (`npm ci`/`npm run build` succeed, `dist/server.js`/`dist/worker.js` both exist); the actual `docker build` itself could **not** be smoke-tested in this sandbox — no Docker Hub registry access from here (`node:20-slim` pulls 403) — flagged rather than claimed.
- **`.github/workflows/ecr-publish.yml`** — builds the `Dockerfile` above and pushes it to Amazon ECR, tagged with the commit SHA, on every push to `main`. Authenticates to AWS via OIDC (`aws-actions/configure-aws-credentials` + `id-token: write`, no long-lived access key stored as a repo secret) — needs three repo secrets this backend's own AWS account has to provision (an ops task, not something this repo can do to itself): `AWS_ROLE_ARN` (an IAM role trusted for `token.actions.githubusercontent.com`, scoped to this repo/branch via its trust policy, with ECR push permissions on the target repository), `AWS_REGION`, and `ECR_REPOSITORY_URI`. **Builds and pushes the image only** — wiring the pushed image into an actual ECS Express Mode service/task definition (or redeploying one on every push) is a separate, not-yet-built step, flagged the same way the App Runner health-check gap was rather than silently left out.
