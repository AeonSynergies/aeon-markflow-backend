...
const emailTemplateSchema = new Schema(
  {
    ...
    // Which WorkflowTemplate.workflow_type category (e.g. "cold_outreach", "win_back") this
    // template is meant for — same free-text convention as persona/workflow_position above (no
    // backing enum anywhere in this codebase for any of the three; values are whatever an org's
    // own workflow_type categories happen to be). Lets the workflow builder's step picker narrow
    // candidates by context — see emailTemplate.service.ts's listEmailTemplatesForOrg.
